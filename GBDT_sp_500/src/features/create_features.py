...
def derive_features(fred_series,sp_data):
    ...
    sp_data = helper_funcs.interpolate_na(sp_data) #Check for NA and interpolate if any found
    ...
    #Corporate spreads
    ig_spread = helper_funcs.interpolate_na(fred_series['ig_spread'])
    hy_spread = helper_funcs.interpolate_na(fred_series['hy_spread'])
    #Change in spreads
    ig_change = ig_spread.diff()*100
    hy_change = hy_spread.diff()*100
    #Change in wti
    wti = helper_funcs.interpolate_na(fred_series['wti'])
    wti_change = wti.pct_change()
    #Yields
    treas_3m = helper_funcs.interpolate_na(fred_series['treas_3m'])
    treas_5y = helper_funcs.interpolate_na(fred_series['treas_5y'])
    treas_10y = helper_funcs.interpolate_na(fred_series['treas_10y'])
    treas_30y = helper_funcs.interpolate_na(fred_series['treas_30y'])
    #Change in yields
    ...
    #Breakeven yields
    binf_5y = helper_funcs.interpolate_na(fred_series['binf_5y'])
    binf_10y = helper_funcs.interpolate_na(fred_series['binf_10y'])
    #Change in breakeven yields
    binf5_change = binf_5y.diff()*100
    binf10_change = binf_10y.diff()*100
    #2s10s and 3m10s
    y10_y2 = helper_funcs.interpolate_na(fred_series['y10_y2'])
    y10_m3 = helper_funcs.interpolate_na(fred_series['y10_m3'])
    #Change
    y10_y2change = y10_y2.diff()*100
    y10_m3change = y10_m3.diff()*100
    # VIX and change
    vix = helper_funcs.interpolate_na(fred_series['vix'])
    vix_change = vix.pct_change()
    #ccy returns
    eur = helper_funcs.interpolate_na(fred_series['eur'])
    jpy = helper_funcs.interpolate_na(fred_series['jpy'])
    gbp = helper_funcs.interpolate_na(fred_series['gbp'])
    cny = helper_funcs.interpolate_na(fred_series['cny'])
    #ccy change
